...
export default function ProgressPage() {
    ...
    const loadProgress = async () => {
      ...
      try {
        const [profileResponse, progressResponse] = await Promise.all([
          ...
          fetch(`${API_BASE_URL}/api/session/progress?weekOffset=${weekOffset}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          }),
        ...
        if (profileData.user?.createdAt) {
          setUserCreatedAt(new Date(profileData.user.createdAt));
        }
  ...
  const handlePrevWeek = () => {
    ...
    const diff = now.getDate() - currentDay + (currentDay === 0 ? -6 : 1) + ((weekOffset - 1) * 7);
    const prevStartOfWeek = new Date(now);
    prevStartOfWeek.setDate(diff);

    const prevEndOfWeek = new Date(prevStartOfWeek);
    prevEndOfWeek.setDate(prevEndOfWeek.getDate() + 7);

    if (prevEndOfWeek <= userCreatedAt) {
      setGoalPopup({ visible: true, message: 'You have reached the week of your account creation.', isError: false });
      window.setTimeout(() => setGoalPopup({ visible: false, message: '', isError: false }), 4000);
      return;
    }
      ...
      <div className="flex min-h-screen">
        ...
        <main className="flex-1 lg:ml-64 p-4 md:p-8">
          <div className="max-w-[1400px] mx-auto space-y-8">
            ...
            {/* Header Section */}
            <header className="flex items-center justify-between gap-4 py-2 animate-in fade-in slide-in-from-top duration-500">
              <div>
                ...
                <section className="rounded-[2.5rem] border border-white bg-white/60 p-8 shadow-sm backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/60 animate-in fade-in duration-1000">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-10 gap-4">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      <button 
                        ...
                        className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-violet-100 dark:hover:bg-violet-900/40 text-slate-500 hover:text-violet-600 transition"
                      >
                        <ChevronLeft size={18} />
                      </button>
                      ...
                      <button 
                        ...
                        className={`p-2 rounded-xl transition ${weekOffset >= 0 ? 'opacity-50 cursor-not-allowed bg-slate-50 dark:bg-slate-800 text-slate-400' : 'bg-slate-50 dark:bg-slate-800 hover:bg-violet-100 dark:hover:bg-violet-900/40 text-slate-500 hover:text-violet-600'}`}
                      >
                  ...
                  <div className="grid grid-cols-7 gap-4 items-end h-64 mt-4">
                    {progress.studyHours.map((entry, i) => {
                      const maxHoursInWeek = Math.max(...progress.studyHours.map(e => e.hours), 1); // Minimum scale of 1 hour
                      const barHeightPercentage = (entry.hours / maxHoursInWeek) * 100;
                      ...
                      <div key={entry.label} className="flex flex-col items-center gap-4 group">
                        <div className="relative w-full flex justify-center">
                           ...
                           <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap bg-slate-900 text-white text-[10px] px-2 py-1 rounded-lg font-bold z-10">
                              {entry.hours >= 1 ? `${Math.floor(entry.hours)}h ` : ''}{Math.round((entry.hours % 1) * 60)}m
                           </div>
                           <div className="w-full max-w-[40px] rounded-2xl bg-slate-100/50 dark:bg-slate-800/50 h-52 flex items-end overflow-hidden group-hover:bg-slate-100 dark:group-hover:bg-slate-800 transition-colors">
                            <div
                              ...
                              style={{ 
                                height: `max(8px, ${barHeightPercentage}%)`,
