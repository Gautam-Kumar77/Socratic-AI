...
export default function TutorDashboardOverviewPage() {
  ...
  const [stats, setStats] = useState({ totalSessions: 0, totalHours: 0, conceptsMastered: 0, focusStreak: 0, streakExpired: false });
  ...
  useEffect(() => {
    const initializePage = async () => {
      ...
      try {
        ...
        const serverSessions = progressData.sessions || [];
        
        // Ensure local storage is synced to the authenticated user's actual history, retaining messages
        const storedHistory = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
        const mergedHistory = serverSessions.map(serverEntry => {
          const localEntry = storedHistory.find(local => 
            (local.sessionId || local.id) === (serverEntry.sessionId || serverEntry._id)
          );
          return {
            ...serverEntry,
            id: serverEntry.sessionId || serverEntry._id,
      ...
      <div className="flex min-h-screen">
        ...
        <aside className="fixed left-0 top-0 bottom-0 w-64 border-r border-slate-200 bg-white/80 backdrop-blur-md px-6 py-6 dark:border-slate-800 dark:bg-slate-950/80 z-20 hidden lg:flex flex-col">
          ...
          <div className="mt-auto border-t border-slate-100 pt-6 dark:border-slate-800">
             <div className={`rounded-2xl p-4 border ${stats.streakExpired ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900/20' : 'bg-violet-50 dark:bg-violet-900/10 border-violet-100 dark:border-violet-900/20'}`}>
               <div className={`flex items-center gap-2 mb-1 ${stats.streakExpired ? 'text-red-600 dark:text-red-400' : 'text-violet-700 dark:text-violet-400'}`}>
                 <Flame size={16} className="fill-current" />
                 <span className="text-xs font-bold uppercase tracking-wider text-[10px]">Learning Streak</span>
               </div>
               <p className="text-xl font-black text-slate-900 dark:text-white">
                 {stats.focusStreak || 0} {stats.focusStreak === 1 ? 'Day' : 'Days'} 🔥
               </p>
               {stats.streakExpired && (
                 <p className="text-xs text-red-600 dark:text-red-400 mt-1 font-medium">Your previous streak has expired. Start studying to build a new one!</p>
               )}
               <div className="mt-2 h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                 <div className={`h-full rounded-full ${stats.streakExpired ? 'bg-red-500' : 'bg-violet-600'}`} style={{ width: `${Math.min((stats.focusStreak || 0) * 10, 100)}%` }} />
               </div>
             </div>
          </div>
        </aside>
        ...
        <main className="flex-1 lg:ml-64 p-4 md:p-8 transition-all">
          <div className="max-w-[1400px] mx-auto space-y-8">
            ...
            {['Physics', 'Chemistry', 'Mathematics', 'Biology', 'Computer'].map((subject) => {
              const icons = {
                ...
                Computer: <Monitor size={24} className={selectedSubject === 'Computer' ? "text-violet-600" : "text-slate-400"} />
              };
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-black">
                            {Math.floor(((stats.totalSeconds || 0) % 3600) / 60)}m {Math.floor((stats.totalSeconds || 0) / 3600) > 0 ? `${Math.floor((stats.totalSeconds || 0) / 3600)}h` : ''} Studied
                          </p>
                        </div>
                      </div>
                    </div>
