...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const initializeDashboard = async () => {
      ...
      try {
        ...
        // Sync local storage with server source of truth for this user, retaining messages
  ...
  const openHistorySession = (entry) => {
    const params = new URLSearchParams({
      ...
      session: entry.sessionId || entry._id,
    });
    router.push(`/dashboard/session?${params.toString()}`);
  };

  const handleDeleteConversation = (entryId) => {
    const nextHistory = historyEntries.filter((entry) => (entry._id || entry.sessionId) !== entryId);
            ...
            <div className="grid grid-cols-12 gap-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {historyEntries.length === 0 ? (
                      ...
                      historyEntries.map((entry, index) => (
                        <div
                          key={entry._id || entry.sessionId || index}
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <div className="relative">
                              <button
                                type="button"
                                onClick={() => setMenuOpenId(menuOpenId === (entry._id || entry.sessionId) ? null : (entry._id || entry.sessionId))}
                                className="rounded-lg p-1 text-slate-400 transition hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800"
                              >
                                <MoreVertical size={16} />
                              </button>
                              {menuOpenId === (entry._id || entry.sessionId) && (
                                <div className="absolute right-0 top-8 z-30 w-32 rounded-xl bg-white p-1 shadow-xl border border-slate-100 dark:bg-slate-900 dark:border-slate-800">
                                  <button
                                    onClick={() => handleDeleteConversation(entry._id || entry.sessionId)}
              ...
              <div className="col-span-12 lg:col-span-4 space-y-6">
                ...
                <section className="rounded-3xl border border-white bg-white/60 p-6 shadow-sm backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/60">
                  ...
                  <div className="space-y-2">
                    {['Physics', 'Mathematics', 'Chemistry', 'Biology', 'Computer'].map((sub) => (
