  ...
  useEffect(() => {
    const initializeSession = async () => {
      ...
      if (progressResponse.ok) {
        ...
        // Merge with local storage to retain messages
        const storedHistory = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
        const mergedHistory = serverHistory.map(serverEntry => {
        ...
        const existingSession = mergedHistory.find((entry) => entry.id === sessionId);
        ...
        const existingSession = storedHistory.find((entry) => (entry.id || entry.sessionId) === sessionId);
        if (existingSession?.messages?.length) {
          setMessages(existingSession.messages);
        }
      }
      ...
      <div className="flex min-h-screen">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            ...
            <div className="rounded-2xl bg-violet-50/50 p-3 border border-violet-100/50 dark:bg-violet-900/10 dark:border-violet-900/20">
               ...
               <p className="text-lg font-black text-slate-900 dark:text-white leading-none">
                 {Math.floor(((stats.totalSeconds || 0) % 3600) / 60)}m {Math.floor((stats.totalSeconds || 0) / 3600) > 0 ? `${Math.floor((stats.totalSeconds || 0) / 3600)}h` : ''}
               </p>
            </div>
          </div>
        ...
        <main className={`flex-1 p-4 transition-all duration-300 md:p-6 ${isFocusMode ? 'md:p-4' : ''}`}>
          <section className={`mx-auto flex min-h-[calc(100vh-2rem)] flex-col rounded-[2rem] border border-violet-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(245,243,255,0.92))] p-4 text-slate-900 shadow-[0_24px_80px_rgba(148,163,184,0.2)] transition-all duration-300 md:p-6 dark:border-white/12 dark:bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(245,243,255,0.95))] dark:shadow-[0_24px_80px_rgba(2,6,23,0.45)] ${isFocusMode ? 'max-w-[1400px]' : 'max-w-[1080px]'}`}>
            <header className="mb-4 flex items-center justify-between gap-4 rounded-[1.6rem] border border-violet-200 bg-white/80 px-5 py-5 shadow-sm dark:border-slate-200/80">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.25em] text-violet-700">Active Tutor Session</p>
                <h1 className="mt-2 text-4xl font-extrabold text-slate-900 dark:text-white">{subject}</h1>
              </div>
              <div className="flex flex-wrap items-center justify-end gap-3">
                <SessionTimer seconds={elapsedSeconds} />
                <FocusModeToggle isActive={isFocusMode} onToggle={toggleFocusMode} />
                <div className="flex items-center gap-2 rounded-full bg-emerald-100 px-4 py-2 text-sm font-semibold text-emerald-800 shadow-sm dark:bg-emerald-900/40 dark:text-emerald-100">
                  <span className="h-3 w-3 rounded-full bg-gradient-to-br from-emerald-300 to-emerald-600 shadow-[inset_-1px_-1px_3px_rgba(0,0,0,0.2),1px_1px_3px_rgba(0,0,0,0.15)] ring-2 ring-emerald-200/50"></span>
                  <span>Session Active</span>
                </div>
                <button
                  onClick={handleEndSession}
                  className="rounded-full border border-violet-300 bg-violet-100 px-5 py-2 text-sm font-semibold text-violet-900 transition hover:bg-violet-200 dark:border-violet-700/50 dark:bg-violet-900/50 dark:text-violet-100 dark:hover:bg-violet-800/60"
                >
                  End Session
                </button>
              </div>
