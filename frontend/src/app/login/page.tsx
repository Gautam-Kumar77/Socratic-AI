...
function LoginContent() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Clear any session data from a previously logged in account
      localStorage.removeItem('socratic-session-history');
      localStorage.removeItem('socratic-active-session');
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 transition-colors duration-500">
                ...
                <form onSubmit={handleRegister} className="space-y-5">
                  ...
                  <label className="flex items-center gap-3 p-2 cursor-pointer group">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 group-hover:text-slate-700">
                      I accept the <span className="text-violet-600 hover:underline hover:text-violet-800" onClick={(e) => { e.preventDefault(); setShowTermsModal(true); }}>Terms and Cognitive Privacy Policy</span>
                    </span>
                  </label>
      ...
      {showTermsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
              <h2 className="text-xl font-black text-slate-800 dark:text-slate-100">Terms and Cognitive Privacy Policy</h2>
              <button 
            ...
            <div className="p-6 overflow-y-auto font-medium text-sm text-slate-600 dark:text-slate-400 space-y-4">
              <p>Welcome to Socratic AI Tutor. By accessing this platform, you agree to our guiding values and operational constraints outlined below.</p>
              
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mt-6">1. Usage Guidelines</h3>
              <p>You agree to use the service exclusively for expanding knowledge and personal enlightenment. Any malicious query or attempt to override the tutor's logic will result in access termination.</p>

              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mt-6">2. Cognitive Privacy</h3>
              <p>Your session interactions, including questions asked and answers received, may be analyzed strictly to personalize your learning paths. Absolute anonymity is maintained.</p>

              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mt-6">3. Content Accuracy</h3>
              <p>While the AI is designed to simulate Socratic inquiry, it is a tool. You must independently verify any critical factual information. The developers bear no liability for intellectual missteps.</p>
              
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mt-6">4. Agreement</h3>
              <p>By closing this notice or checking the acceptance box, you embrace these principles.</p>
            </div>
            <div className="p-6 border-t border-slate-200 dark:border-slate-800 flex justify-end">
              <button 
                onClick={() => setShowTermsModal(false)}
                className="px-6 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded font-bold transition-all"
              >
                Understood
              </button>
            </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 transition-colors duration-500">
        <p className="text-lg text-slate-700 dark:text-slate-200 font-bold">Loading...</p>
      </div>
    }>
      <LoginContent />
    </Suspense>
